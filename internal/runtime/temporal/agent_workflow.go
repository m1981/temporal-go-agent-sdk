...
// Use base.MsgToolRejected, base.MsgToolUnauthorized.
// Temporal has a different unavailable message due to event stream being unavailable.
const (
	...
	for iter := input.State.Iteration; iter < maxIter; iter++ {
		...
		if tokenBudget != nil && llmResult.Usage != nil {
			...
			if tokenBudget.IsExceeded() {
				logger.Warn("workflow: token budget exceeded, stopping agent loop",
					"scope", "workflow",
					"maxTokens", rt.AgentConfig.Limits.MaxTokens,
					"usedTokens", tokenBudget.CurrentUsage().TotalTokens)
...
func (rt *TemporalRuntime) executeAgentToolCall(input agentToolCallInput, tc ToolCallRequest, streamingUnavailable bool) (*agentToolCallOutput, error) {
	...
	return &agentToolCallOutput{
		msg:                  base.BuildToolResultMessage(tc.ToolCallID, tc.ToolName, content),
		failed:               failed,
