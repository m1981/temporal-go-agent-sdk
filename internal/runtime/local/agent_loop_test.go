...
import (
	...
	testutil "github.com/m1981/temporal-go-agent-sdk/internal/testing"
	"github.com/m1981/temporal-go-agent-sdk/internal/types"
	"github.com/m1981/temporal-go-agent-sdk/pkg/interfaces"
	ifmocks "github.com/m1981/temporal-go-agent-sdk/pkg/interfaces/mocks"
...
func TestExecuteSingleTool_AuthorizationDenied(t *testing.T) {
	...
	require.NoError(t, err)
	require.Contains(t, msg.message.Content, base.MsgToolUnauthorized)
...
func TestExecuteSingleTool_ApprovalUnavailable(t *testing.T) {
	...
	require.NoError(t, err)
	require.Contains(t, msg.message.Content, base.MsgToolApprovalUnavailable)
}
...
func TestExecuteSingleTool_ApprovalHandlerRejects(t *testing.T) {
	...
	msg, err := rt.executeSingleTool(context.Background(),
		...
		testToolCallNeedsApproval("c1", "guarded"), noopEmit)
	require.NoError(t, err)
	require.Equal(t, base.MsgToolRejected, msg.message.Content)
}
