...
// Message constants are now in base package to avoid duplication.
// Use base.MsgToolRejected, base.MsgToolApprovalUnavailable, base.MsgToolUnauthorized.
...
func (rt *LocalRuntime) RunAgentLoop(ctx context.Context, input AgentLoopInput) (*AgentLoopResult, error) {
	...
	// Initialize token budget if MaxTokens is configured.
	var tokenBudget *base.TokenBudget
	if rt.AgentConfig.Limits.MaxTokens > 0 {
		tokenBudget = base.NewTokenBudget(rt.AgentConfig.Limits.MaxTokens)
	}
	...
	for iter := 0; iter < maxIter; iter++ {
		...
		// Check token budget.
		if tokenBudget != nil && llmResult.Usage != nil {
			usage := base.ConvertFromLLMUsage(llmResult.Usage)
			tokenBudget.AddUsage(&usage)
			if tokenBudget.IsExceeded() {
				log.Warn(ctx, "local: token budget exceeded, stopping agent loop",
					slog.String("scope", "loop"),
					slog.Int64("maxTokens", rt.AgentConfig.Limits.MaxTokens),
					slog.Int64("usedTokens", tokenBudget.CurrentUsage().TotalTokens))
				telemetry.Run.FinishReason = types.FinishReasonTokenBudget
				break
			}
		}
...
func (rt *LocalRuntime) executeSingleTool(
	...
	if !authResult.Allowed {
		content := base.FormatAuthorizationDeniedMessage(authResult.Reason)
	...
	case types.ApprovalStatusApproved:
		if isSubAgent {
			...
			if canDelegate, msg := base.CheckSubAgentDepth(input.SubAgentDepth, input.MaxSubAgentDepth); !canDelegate {
			...
			if execErr != nil {
				...
				if toolErr.IsFatal {
					...
					log.Info(ctx, "local: tool execution failed (business)",
						slog.String("scope", "loop"),
						slog.String("tool", tc.ToolName),
						slog.String("toolCallID", tc.ToolCallID),
						slog.Any("error", execErr))
	...
	case types.ApprovalStatusRejected:
		content = base.MsgToolRejected
	case types.ApprovalStatusUnavailable:
		content = base.MsgToolApprovalUnavailable
	default:
		return toolResult{
			message: interfaces.Message{},
			failed:  true,
	...
	emitToolEndThenResult(tc.ToolCallID, content)
	return toolResult{
		message: base.BuildToolResultMessage(tc.ToolCallID, tc.ToolName, content),
		failed:  failed,
	}, nil
}
